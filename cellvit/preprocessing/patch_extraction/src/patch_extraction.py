...
class PreProcessor(object):
    ...
    def _check_overwrite(self, overwrite: bool = False) -> None:
        ...
        if overwrite:
            ...
            try:
                ...
                logger.info("JSONDecodeError, likely due to sudden abort of previous run. Restoring from logs.")
                logs = list(Path(self.config.output_path).glob("*preprocessing.log"))
                # TODO: Remove the currently open log file
                processed_files = set()
                # The pattern finds the filename of the file processed which is suceeded by a 
                # "Finished Processing and Storing" line. Note that the suffix is not included in the matched
                # name to match the convension of self.files
                # TODO: This is brittle, find a better way by perhaps examining the actual output files.
                processed_pattern = re.compile(r"Computing patches for ([\w]+)\.{}.*?Finished Processing and Storing.".format(self.config.wsi_extension), flags=re.DOTALL | re.MULTILINE)
                for log_file in logs:
                    with open(log_file, 'r') as fp:
                        # TODO: Replace this with a buffered read so large logs can be handled
                        log_text = fp.read()
                        matches = re.findall(processed_pattern, log_text)
                        processed_files.update(matches)
                self._drop_processed_files(processed_files)
    ...
    def _drop_processed_files(self, processed_files: list[str]) -> None:
        ...
        if processed_files:
            ...
            self.files = [f for f in self.files if f.stem not in processed_files]
            logger.info(f"Dropped {len(processed_files)} files, leaving {len(self.files)} to process")
